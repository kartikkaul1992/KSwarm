...
char ssid[] = "Knet";  //  your network SSID (name)
char pass[] = "9818623664";       // your network password
...
IPAddress serverAddress = IPAddress(0, 0, 0, 0); // default IP Address of Rasbpi on laptop hotspot
...
void loop()
{
  ...
  while(!udp.parsePacket() && (millis()-timestam)<100){
  ...
  analogWrite(D5, map(clearColor, 0, 65536, 0, 255));
    
    }

    if((millis()-timestam)>=100){
      checkAndSetIfMaster();
    }
...
void sendLogToServer()
{
  ...
  if (masterState == true)
  {
    ...
    udp.beginPacket(serverAddress,  localPort1); //
